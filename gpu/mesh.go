...
func (context *Context) NewMesh(a ...Attribute) (Mesh, error) {
	...
	for i := range mesh.Triangles() {
		...
		var vertices = [3]vec3.Type{a, b, c}
...
func (m Mesh) Triangle(i int) (a, b, c vec3.Type) {
	...
	if m.indexed {
		...
		return vec3.Type(buf.vertices[m.voffset+buf.indicies[offset]]),
			vec3.Type(buf.vertices[m.voffset+buf.indicies[offset+1]]),
			vec3.Type(buf.vertices[m.voffset+buf.indicies[offset+2]])
	}
	offset := i*3 + int(m.offset)
	return vec3.Type(buf.vertices[offset]), vec3.Type(buf.vertices[offset+1]), vec3.Type(buf.vertices[offset+2])
}
